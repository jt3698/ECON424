...
```{r}
# top 10 of each
key_pc2 <- findMostFreqTerms(dtm, 10, INDEX = rep(1, each = n))
key_gen2 <- findMostFreqTerms(dtm2, 10, INDEX = rep(1, each = n))
key_cast2 <- findMostFreqTerms(dtm3, 10, INDEX = rep(1, each = n))
...
```{r}
coef_sort2 <- sort(coef2[,1], decreasing = TRUE)[-1]
head(coef_sort2, 10)
tail(coef_sort2, 10)
```
