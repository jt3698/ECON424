...
coef_head <- head(coef_sort, 10)
coef_tail <- tail(coef_sort, 10)
int_vars <- c("p$screen_gems", "p$walt_disney_pictures", "p$paramount", "p$new_line_cinema", "p$columbia_pictures", "p$touchstone_pictures", "p$universal_pictures", "p$20th_century_fox", "p$tristar_pictures", "p$hollywood_pictures", "k$film_noir", "l$fa", "k$pro_wrestling", "l$ur", "k$lgbt")
...
for(i in int_vars){
  for (j in int_vars){
    if (i < j){
      v = rep(0, n)
      var_name = paste(c(i,j), sep = ".")
      for(k in 1:n){
        v[k] = as.numeric(a[k]) * as.numeric(b[k])
      }
      inact <- cbind(inact, v)
...
df_inact = data.frame(inact, colnames = inact_name)
```
