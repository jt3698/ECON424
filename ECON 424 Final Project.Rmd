...
```{r}
X <- cbind(dtm[, key_pc], dtm2[, key_gen], dtm3[, key_cast], dtm4[, key_keys], dtm5[, key_lang])
y <- log(clean_data$revenue)
library(glmnet)
set.seed(424)
model1 <- cv.glmnet(cbind(as.matrix(X), budget = log(clean_data$budget)), y)
coef1 <- coef(model1, s = "lambda.min")
sort(coef1[,1], decreasing = TRUE)
```

Take 10 highest coef and 10 lowest coef, then do interactions on them.
...
```{r}
# top 10 of each
key_pc2 <- sort(findFreqTerms(dtm, 200))
key_gen2 <- sort(findFreqTerms(dtm2, 1215))
key_cast2 <- sort(findFreqTerms(dtm3, 69))
key_cast2 <- key_cast2[grepl('_', key_cast2)]
key_keys2 <- sort(findFreqTerms(dtm4, 340))
...
```{r}
X2 <- cbind(dtm[, key_pc], dtm2[, key_gen], dtm3[, key_cast], dtm4[, key_keys], dtm5[, key_lang], df_inact)
y <- log(clean_data$revenue)
model2 <- cv.glmnet(cbind(as.matrix(X2), budget = log(clean_data$budget)), y)
coef2 <- coef(model2, s = "lambda.min")
sort(coef2[,1], decreasing = TRUE)
```



```{r}
coef_sort2 <- sort(coef2[,1], decreasing = TRUE)
head(coef_sort2, 10)
tail(coef_sort2, 10)
```
...
for (df in df_list){
  ...
  spc <- paste(sub_prod_comp, collapse="-")
  sub_docs <- Corpus(VectorSource(spc))
  # Remove "-" from the name since it is the splitter symbol of the data
  ...
  sub_key_pc <- sort(findMostFreqTerms(sub_dtm, 10))
  #print(sub_key_pc)
  
  #### GENRES
  sub_genres <- df$genres
  sg <- paste(sub_genres, collapse="-")
  sub_docs2 <- Corpus(VectorSource(sg))
  ...
  #get top 10 genres
  sub_key_gen <- sort(findMostFreqTerms(sub_dtm2, 10))
  #sub_key_gen
  
  #### CREDITS
  sub_casts <- df$credits
  sc <- paste(sub_casts, collapse="-")
  sub_docs3 <- Corpus(VectorSource(sc))
  ...
  #get top 10 casts
  sub_key_cast <- sort(findMostFreqTerms(sub_dtm3, 10))

  #remove single-word names
  sub_key_cast <- sub_key_cast[grepl('_', sub_key_cast)]
  #sub_key_cast
  
  #### KEYWORDS
  sub_keywords <- df$keywords
  sk <- paste(sub_keywords, collapse="-")
  sub_docs4 <- Corpus(VectorSource(sk))
  ...
  #gettop 10 keywords
  sub_key_keys <- sort(findMostFreqTerms(sub_dtm4, 10))
  #sub_key_keys
  
  #### ORIGINAL LANGUAGE
  sub_og_lng <- df$original_language
  sol <- paste(sub_og_lng, collapsse = "-")
  sub_docs5 <- Corpus(VectorSource(sol))
  # adding prefix l$ for language
  ...
  #get all languages in at least 50 movies
  sub_key_lang <- sort(findMostFreqTerms(sub_dtm5, 10))
  #sub_key_lang
